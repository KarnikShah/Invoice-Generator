...
if(isset($_GET['type']) && $_GET['type']=='email'){
    ...
    $mpdfa = new \Mpdf\Mpdf();
    $mpdf->WriteHTML($html);
    $mpdfa->WriteHTML($html1);
    $file = time() . '.pdf';
    $pdf=$mpdf->Output($file, 's'); 
    $chalan=$mpdfa->Output($file, 's'); 
    echo smtp_mailer('test@gmail.com','Invoice For Recent Purchase',$pdf,$chalan,"Thank you for purchasing items from Mas Packaging. Invoice of purchase items is attached in the email. If you have any queries please free to contact us."); 
}
?>